...
public class MixinSkipSyncRegistry {
    ...
    private static boolean skipRegistryWithoutModded(RegistryAttributeHolder instance, RegistryAttribute registryAttribute, Operation<Boolean> original, @Local(name = "registry") Registry<?> registry) {
        if (registryAttribute == RegistryAttribute.MODDED) {
            if (registry.keySet().stream().noneMatch(identifier -> !identifier.getNamespace().equals("minecraft") && !identifier.getNamespace().equals("galaxy"))) {
                return false;
            }
        }
