...
class GalaxyManager {
    ...
    fun saveGalaxy(galaxy: Galaxy) = launch { collection.replaceOne(eq("uuid", galaxy.uuid), galaxy) }
    ...
    fun getGalaxy(uuid: UUID): Deferred<Galaxy?> = async { collection.find(eq("uuid", uuid)).first() }

    fun getGalaxy(planet: Planet) = async { collection.find(eq("planets.uuid", planet.uuid)).first() }
